...
\begin{enumerate}
\item[(a)] How many scrambled boards are there for the $1\times 2$ Spinpossible game?  Don't forget to include the solved board.
\item[(b)] How many actions are there in $\Spin_{1\times 2}$? Which of these actions are spins? \emph{Hint:} There are actions that are not spins.
\end{enumerate}
\end{exercise}
...
If we include the rest of the scrambled boards and all possible spins, we obtain Figure~\ref{fig:Cayley_Spin_1x2}. Note that I've chosen a nice layout for the figure, but it's really the connections between the various boards that is important.
...
In order to obtain the Cayley diagram for $\Spin_{1\times 2}$, we need to identify each scrambled board in Figure~\ref{fig:Cayley_Spin_1x2} with an action from the group.  The most natural choice is to identify the solved board with the do-nothing action, which we will denote by $e$.  As soon as we make this choice, we can just follow the arrows around the diagram to determine which actions correspond to which scrambled boards.  
...
\end{tikzpicture}
\end{center}
To get here from the scrambled board, we can simply follow a green arrow. So, this scrambled board corresponds to $t_1$.  However, we could also follow a red arrow, then a blue arrow, and then a red arrow. Thus, we could also label the scrambled board by $s t_2 s$.
...
Label each of the remaining boards from Figure~\ref{fig:Cayley_Spin_1x2} with at least one appropriate word using $t_1, t_2$, and $s$.  The diagram in Figure~\ref{fig:Cayley_Spin_1x2} together with your labels is the Cayley diagram for $\Spin_{1\times 2}$.
\end{exercise}
...
The arrows follow the direction of the action that it corresponds to. If a generator is its own reversal, then the arrows corresponding to that generator are two-way arrows.  It is always true that following an arrow backwards corresponds to a generator's reversal.  That is, if an arrow corresponds to the action $a$, then the inverse of $a$, namely $a^{-1}$, corresponds to the reverse arrow. We need a way to tell our arrow types apart.  One way to do this is to color them.  Another way would be to label the arrows by their corresponding generator.
...
In the Cayley diagram for $\Spin_{1\times 2}$, our vertices were fancy pictures of scrambled $1\times 2$ Spinpossible boards.  This wasn't necessary, but is convenient and appealing for aesthetic reasons.  After labeling the solved board with the do-nothing action, $e$, in Exercise~\ref{exer:Cayley_Spin_1x2} you labeled each remaining vertex of the diagram with a word that corresponds to a sequence of arrows from the solved board to the vertex in question.
...
\item[(c)] Which of your words above is the reversal of $r$?  That is, can we describe $r^{-1}$ using $r$?
...
\item[(b)] Let $r$ be rotation by $120^\circ$ in the clockwise direction.  Does $r$ generate $D_3$? That is, can you write each of your actions from part (a) as words in $r$?
\item[(c)] What is the reversal of $r$?  That is, what is $r^{-1}$? Can you write it as a word in $r$?
...
\item[(e)] What is the reversal of $s$?  That is, what is $s^{-1}$? Can you write it as a word?
\item[(f)] Can we generate all of $D_3$ using both $r$ and $s$?  If so, write all the actions of $D_3$ as words in $r$ and $s$ (or their reversals/inverses).
