...
\noindent This board is one of our 8 possible scrambled boards in $\Spin_{1\times 2}$.  We have three possible spins we can do to this board: toggle position 1, toggle position 2, or spin the whole board.  Each of these spins has a corresponding two-way arrow that takes us to three different scrambled boards.  Figure~\ref{fig:partial_Cayley} provides a visual of what we just discussed.
...
\end{tikzpicture}
\end{center}
Looking at our current diagram for $\Spin_{1\times 2}$, we see that one way to get to this board from the solved board is to follow a blue arrow and then a red arrow.  This corresponds to the word $s_1t_2$.  (Recall that when we write down words, we should apply the actions from right to left, just like function composition.) However, it also corresponds to the word $t_2s_1t_2t_1$ even though this is not an optimal solution.  So, we can label the board in question with either $s_1t_2$ or $t_2s_1t_2t_1$ (there are other choices, as well).
...
\end{tikzpicture}
\end{center}
To get here from the scrambled board, we can simply follow a green arrow. So, this scrambled board corresponds to $t_1$.  However, we could also follow a red arrow, then a blue arrow, and then a red arrow. Thus, we could also label the scrambled board by $s_1 t_2 s_1$.

\begin{exercise}
Using Figure~\ref{fig:Cayley_Spin_1x2}, find three distinct words in $t_1, t_2$, and $s_1$ that correspond to the following scrambled board.  Don't worry about whether your word is of optimal length or not.
\begin{center}
\begin{tikzpicture}[every node/.style={minimum size=.65cm}]
  \node [draw] (1) {\rotatebox{180}{$\underline{1}$}};
  \node [draw, right=0cm of 1] (2) {\rotatebox{180}{$\underline{2}$}};
...
\begin{exercise}
Label each of the remaining boards from Figure~\ref{fig:Cayley_Spin_1x2} with an appropriate word using $t_1, t_2$, and $s_1$.  The diagram in Figure~\ref{fig:Cayley_Spin_1x2} together with your labels is the Cayley diagram for $\Spin_{1\times 2}$.
\end{exercise}
