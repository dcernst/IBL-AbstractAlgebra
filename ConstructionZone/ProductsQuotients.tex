...
\begin{section}{Quotients of Groups}\label{sec:Quotient Groups}
...
$1$ & \cellcolor{cb-lightorange}$1$ & \cellcolor{cb-lightorange}$-1$ & \cellcolor{cb-orange}$i$ & \cellcolor{cb-orange}$-i$ & \cellcolor{cb-purple}$j$ & \cellcolor{cb-purple}$-j$ & \cellcolor{cb-lightpurple}$k$ & \cellcolor{cb-lightpurple}$-k$\\
\hline $-1$ & \cellcolor{cb-lightorange}$-1$ & \cellcolor{cb-lightorange}$1$ & \cellcolor{cb-orange}$-i$ & \cellcolor{cb-orange}$i$ & \cellcolor{cb-purple}$-j$ & \cellcolor{cb-purple}$j$ & \cellcolor{cb-lightpurple}$-k$ & \cellcolor{cb-lightpurple}$k$ \\
\hline $i$ & \cellcolor{cb-orange}$i$ & \cellcolor{cb-orange}$-i$ & \cellcolor{cb-lightorange}$-1$ & \cellcolor{cb-lightorange}$1$ & \cellcolor{cb-lightpurple}$k$ & \cellcolor{cb-lightpurple}$-k$ & \cellcolor{cb-purple}$-j$ & \cellcolor{cb-purple} $j$\\
\hline $-i$ & \cellcolor{cb-orange}$-i$ & \cellcolor{cb-orange}$i$ & \cellcolor{cb-lightorange}$1$ & \cellcolor{cb-lightorange}$-1$ & \cellcolor{cb-lightpurple}$-k$ & \cellcolor{cb-lightpurple}$k$ & \cellcolor{cb-purple}$j$ & \cellcolor{cb-purple}$-j$\\
\hline $j$ & \cellcolor{cb-purple}$j$ & \cellcolor{cb-purple}$-j$ & \cellcolor{cb-lightpurple}$-k$ & \cellcolor{cb-lightpurple}$k$ & \cellcolor{cb-lightorange}$-1$ & \cellcolor{cb-lightorange}$1$ & \cellcolor{cb-orange}$i$ & \cellcolor{cb-orange}$-i$\\
\hline $-j$ & \cellcolor{cb-purple}$-j$ & \cellcolor{cb-purple}$j$ & \cellcolor{cb-lightpurple}$k$ & \cellcolor{cb-lightpurple}$-k$ & \cellcolor{cb-lightorange}$1$ & \cellcolor{cb-lightorange}$-1$ & \cellcolor{cb-orange}$-i$ & \cellcolor{cb-orange}$i$\\
\hline $k$ & \cellcolor{cb-lightpurple}$k$ & \cellcolor{cb-lightpurple}$-k$ & \cellcolor{cb-purple}$j$ & \cellcolor{cb-purple}$-j$ & \cellcolor{cb-orange}$-i$ & \cellcolor{cb-orange}$i$ & \cellcolor{cb-lightorange}$-1$ & \cellcolor{cb-lightorange}$1$\\
\hline $-k$ & \cellcolor{cb-lightpurple}$-k$ & \cellcolor{cb-lightpurple}$k$ & \cellcolor{cb-purple}$-j$ & \cellcolor{cb-purple}$j$ & \cellcolor{cb-orange}$i$ & \cellcolor{cb-orange}$-i$ & \cellcolor{cb-lightorange}$1$ & \cellcolor{cb-lightorange}$-1$
\end{tabular}
\end{center}
...
\tikzstyle{greenvert} = [circle, draw, fill=cb-green,inner sep=0pt, minimum size=6.5mm]
...
\node (iK) at (45:1.5) [greenvert] {\scriptsize {$Ki$}};
\node (kK) at (-45:1.5) [greenvert] {{\scriptsize $Kk$}};
\node (jK) at (-135:1.5) [greenvert] {{\scriptsize $Kj$}};
\path[p2] (K) to (iK);
\path[o2] (iK) to (kK);
\path[p2] (kK) to (jK);
\path[o2] (jK) to (K);
\end{tikzpicture}}
\caption{The left subfigure shows the Cayley diagram for $Q_8$ with generating set $\{i,j,-1\}$. The right subfigure shows the collapsed Cayley diagram for $Q_8$ according to the right cosets of $K=\langle -1\rangle$.}
\label{fig:Q8Repeat}
\end{figure}

We can visualize the right cosets of $K$ as the four clumps of vertices connected together with the two-way green arrows. These are the clones of $K$. In this case, we are also seeing the left cosets since $K$ is normal in $Q_8$.  If we collapse the right cosets onto each other and collapse the corresponding arrows, we obtain the diagram given in Figure~\ref{fig:Q8QuotientB}. It is clear that this diagram is the Cayley diagram for a group that is isomorphic to $V_4$.  

This process of collapsing a Cayley diagram for a group $G$ according to the right cosets of a subgroup $H$ is called the \textbf{quotient process}.  The vertices in the resulting diagram are the right cosets (i.e., clones) of $H$. If there is an arrow from $g_1$ to $g_2$ in the Cayley diagram for $G$ such that $g_1$ and $g_2$ are in different right cosets, then there is an arrow of the same color from the right coset containing $g_1$ (i.e., $Hg_1$) to the right coset containing $g_2$ (i.e., $Hg_2$).

For reasons we will understand shortly, applying the quotient process to $G$ according to a subgroup $H$ results in the Cayley diagram for a group if and only if $H$ is normal in $G$.
...
The problem that arises in Problem~\ref{prob:bad_quotient} is that if the same arrow types (i.e., those representing the same generator) leaving a right coset do not point at elements in the same right coset, attempting the quotient process will result in a diagram that cannot be a Cayley diagram for a group since we have more than one arrow of the same type leaving a vertex.  In Figure~\ref{fig:QuotientProcessBad}, we illustrate what goes wrong if all the arrows for a generator pointing out of a right coset (i.e., clone) do not unanimously point to elements in the same right coset.  In Figure~\ref{fig:QuotientProcessGood}, all the arrows point to elements in the same right coset, and in this case, it appears that everything works out just fine.
...
\node at (0,-.15) {\scriptsize $Hg_2$};
\node at (2,-.15) {\scriptsize $Hg_3$};
\node at (1,1.9) {\scriptsize $Hg_1$};
...
\node (g1H) at (.8,1.5) [vert] {\scriptsize $Hg_1$};
\node (g2H) at (0,0) [vert] {\scriptsize $Hg_2$};
\node (g3H) at (1.6,0) [vert] {\scriptsize $Hg_3$};
...
\makecell{
\begin{tikzpicture}[scale=1]
\node (g1H) at (0,1.5) [vert] {\scriptsize $Hg_1$};
\node (g2H) at (0,0) [vert] {\scriptsize $Hg_2$};
\draw[p] (g1H) to (g2H);
\end{tikzpicture}}
\end{tabular}
}
\caption{In the left subfigure, blue arrows go from elements of the right coset $Hg_1$ to elements of \emph{multiple} left cosets, which results in ambiguous blue arrows in the diagram resulting from the quotient process. This implies that right coset multiplication is not well defined in this case. In the right subfigure, blue arrows go from elements of the right coset $Hg_1$ to elements inside a \emph{unique} right coset, which does not result in any ambiguity in the diagram resulting from the quotient process.}
