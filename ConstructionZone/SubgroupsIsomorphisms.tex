...
\begin{problem}
Consider the group $D_4$. Compute the image of each element of $D_4$ under each of the following conjugation maps.
\begin{enumerate}[label=\textrm{(\alph*)}]
\item $\phi_r:D_4\to D_4$ defined via $\phi_r(x)=rxr^{-1}$.
\item $\phi_s:D_4\to D_4$ defined via $\phi_s(x)=sxs^{-1}$.
\end{enumerate}
\end{problem}
