...
\begin{problem}
We have seen two different Cayley diagrams for $D_3$, one with generating set $\{s,r\}$ and one with generating set $\{s,s'\}$. As Figure~\ref{fig:D3andS3} illustrates, there is a matching between $D_3$ and $S_3$ that relies on the with generating sets $\{s,s'\}$ and $\{s_1,s_2\}$, respectively. Find a different matching between $D_3$ and $S_3$ that utilizes the generating set $\{r,s\}$ for $D_3$.
\end{problem}

The next theorem follows immediately from the definition of matching.

\begin{theorem}
If there is matching between $G_1$ and $G_2$ using the generating sets $T_1$ and $T_2$, respectively, then $|G|=|G_2|$ and $T_1$ and $T_2$ have the same cardinality.
\end{theorem}
