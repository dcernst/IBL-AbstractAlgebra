...
$e$ & $e$ & $s$ & $sr$ & $r^2$ & $rs$ & $r$\\
\hline $s$ & $s$ & $e$ & $r$ & $rs$ & $r^2$ & $sr$ \\
\hline \cellcolor{lightgray}$sr$ & \cellcolor{cb-purple}$sr$ & \cellcolor{cb-purple}$r^2$ & $e$ & $s$ & $r$ & $rs$\\
\hline $r^2$ & $r^2$ & $sr$ & $rs$ & $r$ & $s$ & $e$\\
\hline $rs$ & $rs$ & $r$ & $r^2$ & $sr$ & $e$ & $s$\\
\hline $r$ & $r$ & $rs$ & $s$ & $e$ & $sr$ & $r^2$\\
\end{tabular}
...
\cellcolor{lightgray}$e$ & $e$ & $s$ & \cellcolor{cb-purple}$sr$ & $r^2$ & $rs$ & $r$\\
\hline \cellcolor{lightgray}$s$ & $s$ & $e$ & \cellcolor{cb-purple}$r$ & $rs$ & $r^2$ & $sr$ \\
\hline $sr$ & $sr$ & $r^2$ & $e$ & $s$ & $r$ & $rs$\\
\hline $r^2$ & $r^2$ & $sr$ & $rs$ & $r$ & $s$ & $e$\\
\hline $rs$ & $rs$ & $r$ & $r^2$ & $sr$ & $e$ & $s$\\
\hline $r$ & $r$ & $rs$ & $s$ & $e$ & $sr$ & $r^2$\\
...
\begin{tabular}{c!{\vrule width 2pt}c|c|c|c|c|c}
$*$ & $e$ & $s$ & $sr$ & $r^2$ & $rs$ & $r$ \\ \noalign{\hrule height 2pt}
$e$ & \cellcolor{cb-orange}$e$ & \cellcolor{cb-orange}$s$ & \cellcolor{cb-purple}$sr$ & \cellcolor{cb-purple}$r^2$ & \cellcolor{green}$rs$ & \cellcolor{green}$r$\\
\hline $s$ & \cellcolor{cb-orange}$s$ & \cellcolor{cb-orange}$e$ & \cellcolor{green}$r$ & \cellcolor{green}$rs$ & \cellcolor{cb-purple}$r^2$ & \cellcolor{cb-purple}$sr$ \\
\hline $sr$ & \cellcolor{cb-purple}$sr$ & \cellcolor{cb-purple}$r^2$ & \cellcolor{cb-orange}$e$ & \cellcolor{cb-orange}$s$ & \cellcolor{green}$r$ & \cellcolor{green}$rs$\\
\hline $r^2$ & \cellcolor{cb-purple}$r^2$ & \cellcolor{cb-purple}$sr$ & \cellcolor{green}$rs$ & \cellcolor{green}$r$ & \cellcolor{cb-orange}$s$ & \cellcolor{cb-orange}$e$\\
\hline $rs$ & \cellcolor{green}$rs$ & \cellcolor{green}$r$ & \cellcolor{cb-purple}$r^2$ & \cellcolor{cb-purple}$sr$ & \cellcolor{cb-orange}$e$ & \cellcolor{cb-orange}$s$\\
\hline $r$ & \cellcolor{green}$r$ & \cellcolor{green}$rs$ & \cellcolor{cb-orange}$s$ & \cellcolor{cb-orange}$e$ & \cellcolor{cb-purple}$sr$ & \cellcolor{cb-purple}$r^2$\\
\end{tabular}
\end{center}
...
\begin{theorem}
Let $G$ be a group, $H\leq G$, and $a\in G$.  Define $\phi:H\to aH$ via $\phi(h)=ah$.  Then $\phi$ is a bijection.
\end{theorem}
