...
\begin{section}{Group Tables}\label{sec:Group Tables}
...
\begin{center}
\begin{tabular}{c!{\vrule width 2pt}c|c|c|c}
$\circ$ & \cellcolor{cb-lightorange}$e$ & \cellcolor{cb-orange}$v$ & \cellcolor{cb-purple}$h$ & \cellcolor{cb-lightpurple}$vh$ \\ \noalign{\hrule height 2pt}
\cellcolor{cb-lightorange}$e$ & \cellcolor{cb-lightorange}$e$ & \cellcolor{cb-orange}$v$ & \cellcolor{cb-purple}$h$ & \cellcolor{cb-lightpurple}$vh$ \\
\hline \cellcolor{cb-orange}$v$ & \cellcolor{cb-orange}$v$ & \cellcolor{cb-lightorange}$e$ & \cellcolor{cb-lightpurple}$vh$ & \cellcolor{cb-purple}$h$  \\
\hline \cellcolor{cb-purple}$h$ & \cellcolor{cb-purple}$h$ & \cellcolor{cb-lightpurple}$vh$ & \cellcolor{cb-lightorange}$e$ & \cellcolor{cb-orange}$v$\\
\hline \cellcolor{cb-lightpurple}$vh$ & \cellcolor{cb-lightpurple}$vh$ & \cellcolor{cb-purple}$h$ & \cellcolor{cb-orange}$v$ & \cellcolor{cb-lightorange}$e$
\end{tabular}
\hspace{1cm}
$\longleftrightarrow$
\hspace{1cm}
\begin{tabular}{c!{\vrule width 2pt}c|c|c|c}
$*$ & \cellcolor{cb-lightorange}$\; e\; $ & \cellcolor{cb-orange}$\; a\;$ & \cellcolor{cb-purple}$\; b\;$ & \cellcolor{cb-lightpurple}$\; c\;$ \\ \noalign{\hrule height 2pt}
\cellcolor{cb-lightorange}$\; e\;$ & \cellcolor{cb-lightorange}$e$ & \cellcolor{cb-orange}$a$ & \cellcolor{cb-purple}$b$ & \cellcolor{cb-lightpurple}$c$ \\
\hline \cellcolor{cb-orange}$a$ & \cellcolor{cb-orange}$a$ & \cellcolor{cb-lightorange}$e$ & \cellcolor{cb-lightpurple}$c$ & \cellcolor{cb-purple}$b$  \\
\hline \cellcolor{cb-purple}$b$ & \cellcolor{cb-purple}$b$ & \cellcolor{cb-lightpurple}$c$ & \cellcolor{cb-lightorange}$e$ & \cellcolor{cb-orange}$a$\\
\hline \cellcolor{cb-lightpurple}$c$ & \cellcolor{cb-lightpurple}$c$ & \cellcolor{cb-purple}$b$ & \cellcolor{cb-orange}$a$ & \cellcolor{cb-lightorange}$e$
\end{tabular}
\end{center}
...
\noindent Note that I could have drawn the four scrambled boards in Figure~\ref{fig:partial_Cayley} anywhere I wanted to, but I have a particular layout in mind.  Also, notice we have three different colored arrows. In this case, a {\color{cb-green}green} arrow corresponds to toggling the tile in position 1 ($s_{11}$), a {\color{cb-purple}purple} arrow corresponds to toggling position 2 ($s_{22}$), and a {\color{cb-orange}orange}  arrow corresponds to spinning the whole board ($s_{12}$). 
...
\centering
\begin{tikzpicture}[scale=1.5,auto]
\node (A) at (-1,2) [vert-superfat] {\scriptsize $e$};
\node (B) at (1,2) [vert-superfat] {\scriptsize $s_{22}$};
\node (C) at (2,.75) [vert-superfat] {\scriptsize $s_{12}s_{22}$};
\node (D) at (2,-.75) [vert-superfat] {\scriptsize $s_{22}s_{12}s_{22}$};
\node (E) at (1,-2) [vert-superfat] {\scriptsize $s_{11}s_{22}$};
\node (F) at (-1,-2) [vert-superfat] {\scriptsize $s_{11}$};
\node (G) at (-2,-.75) [vert-superfat] {\scriptsize $s_{22}s_{12}$};
\node (H) at (-2,.75) [vert-superfat] {\scriptsize $s_{12}$};
\path[p2] (A) to (B);
\path[p2] (C) to (D);
\path[p2] (E) to (F);
\path[p2] (G) to (H);
...
\begin{problem}\label{prob:two groups of order 12}
...
\node (aab) at (2,1.25) [vert] {\scriptsize $s^2t$};
...
\item For each element $g$, find the smallest positive exponent $k$ such that $g^k=e$.
\end{enumerate}
\end{problem}
...
\draw [o] (e) to [bend left] (r);
\draw [o] (r) to [bend left] (rr);
\draw [o] (rr) to [bend left] (e);
\draw [o] (h) to [bend right] (hr);
\draw [o] (hr) to [bend right] (hrr);
\draw [o] (hrr) to [bend right] (h);
\draw [p2] (e) to (h);
\draw [p2] (r) to (hrr);
\draw [p2] (rr) to (hr);
\end{tikzpicture}
...
\begin{enumerate}[label=\textrm{(\alph*)}]
\item If $G$ is finite, what conclusions can you make about the Cayley diagram for $G$ with generating set $\{g\}$?
\item If $G$ is infinite, what conclusions can you make about the Cayley diagram for $G$ with generating set $\{g\}$?
\end{enumerate}
\end{problem}
