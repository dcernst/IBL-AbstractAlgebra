...
\begin{problem}\label{prob:introducing_S2}
Consider the symmetric group $S_2$ consisting of the net actions that permute the positions of two coins (without flipping them over) that are sitting side by side in a line.  Let $s$ be the action that swaps the positions of the two coins.
\begin{enumerate}[label=\rm{(\alph*)}]
