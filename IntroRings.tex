...
\begin{exercise}
Justify that $\mathbb{Z}$ is a commutative ring with 1 under the usual operations of $+$ and $\cdot$. Which elements have multiplicative inverses in $\mathbb{Z}$?
\end{exercise}

\begin{exercise}
Justify that $\mathbb{Z}_n$ is a commutative ring with 1 under $+$ mod $n$ and $\cdot$ mod $n$.
\end{exercise}
...
\begin{enumerate}[label=\rm{(\alph*)}]
\item Argue that $S$ a subring of $R$.
...
We have shown that the quotient $R/I$ of the ring $R$ by a subgroup $I$ has a natural ring structure iff $I$ is closed under multiplication on the left and right by elements of $R$ (which also forces $I$ be a subring).  Such subrings are called \textbf{ideals}.
...
\begin{definition}
An ideal $M$ in a ring $R$ is called a \textbf{maximal ideal} if $M\neq R$ and the only ideals containing $M$ are $M$ and $R$.
\end{definition}
...
\begin{theorem}
Assume $R$ is commutative.  The ideal $M$ is maximal iff $R/M$ is a field.
\end{theorem}
...
\begin{theorem}
Assume $R$ is a commutative ring.  Then the ideal $P$ is a prime ideal in $R$ iff the quotient ring $R/P$ is an integral domain.
\end{theorem}

\begin{corollary}
Assume $R$ is a commutative ring.  Every maximal ideal of $R$ is a prime ideal.
\end{corollary}
