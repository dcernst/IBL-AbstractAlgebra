...
For each quotient group below, describe the group.  If possible, state what group each is isomorphic to.  You may assume that we are taking the quotient by a normal subgroup. 
\begin{enumerate}
\item[(h)] $(\mathbb{Z}_4\times \mathbb{Z}_6)/\langle (0,2)\rangle$
\item[(k)] $(\mathbb{Z}\times \mathbb{Z})/\langle (1,1)\rangle$
\item[(j)] $S_4/\langle (1,2)(3,4),(1,3)(2,4)\rangle$
\item[(i)] $\mathbb{Q}/\langle 1\rangle$ (the operation on $\mathbb{Q}$ is addition)
\end{enumerate}
\end{exercise}
